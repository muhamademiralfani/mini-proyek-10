        ...
        <div className='container mx-auto px-4 sm:px-6 lg:px-8'>
          <div className='flex flex-col lg:flex-row items-center justify-between'>
            <div className='mb-8 lg:mb-0 lg:mr-8 w-full'>
              <h2 className='text-4xl font-bold mb-2'>New In</h2>
              <h2 className='text-4xl font-bold mb-4'>Store Now</h2>
            ...
            <div className='lg:absolute lg:left-[40%] transform'>
              <div className='flex gap-4 overflow-x-auto scrollbar-hide' ref={scrollContainerRef} onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseLeave} onWheel={handleWheel}>
                {status === 'loading' && (
                ...
                {categories.map((category, index) => (
                  <div className='relative lg:min-w-[200px] flex-shrink-0 group' key={index}>
                    <img src={category.image} alt={category.title} className='w-96 h-[500px] rounded-lg transform transition duration-300 group-hover:scale-105' />
                    <div className='absolute inset-0 bg-black bg-opacity-50 opacity-0 group-hover:opacity-100 transition duration-300'></div>
                    <span className='absolute bottom-2 left-2 text-white text-center px-2 py-4 w-full bg-transparent text-base font-medium'>{category.title}</span>
                  </div>
                ))}
              </div>
            </div>
