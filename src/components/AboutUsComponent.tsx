          ...
          {status === 'succeeded' && testimonials.length > 0 && (
            <div className='mb-8'>
              ...
              <div className='flex gap-2'>
                <button
                  aria-label='previous'
                  className={`w-10 h-10 flex items-center justify-center rounded-full shadow-md border ${page === 1 ? 'bg-white text-gray-400 cursor-not-allowed' : 'bg-white text-black hover:shadow-lg'}`}
                  onClick={handlePrev}
                  disabled={page === 1}>
                ...
                <button
                  aria-label='next'
                  className={`w-10 h-10 flex items-center justify-center rounded-full shadow-md ${page === totalPages ? 'bg-teal-300 text-white cursor-not-allowed' : 'bg-teal-600 text-white hover:bg-teal-700'}`}
                  onClick={handleNext}
