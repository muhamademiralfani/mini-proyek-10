...
const WeCreateComponent: React.FC = () => {
  ...
  return (
    <section
      className="flex lg:flex-row flex-col-reverse container max-w-screen-xl mx-auto min-h-screen items-center gap-12 px-4 py-16"
          ...
          {services.map((service, index) => (
            <div className="flex gap-4" key={index} data-testid={`service-item-${index}`}>
              <div className="flex-shrink-0">
                <img
                  src={checklist}
                  alt={`Checklist icon for ${service.title}`}
